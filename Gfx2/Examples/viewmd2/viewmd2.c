...
Shader shaderShadow; 
Uniform shaderShadow_unis[3];
Vector4f testcolor = {1.0,0.0,0.0,1.0};

unsigned char *md2_data;
unsigned char curr_frame = 40;
...
void setShaders() {
  ...
  shader_vert = shader_load("Shaders/const_mv.vert",
  				 GL_VERTEX_SHADER);
  shader_frag = shader_load("Shaders/const_mv.frag",
  				 GL_FRAGMENT_SHADER);
  ...
  shader_uniform_v4f(&shaderShadow_unis[2],shader_color,&testcolor);

  shaderShadow.uniforms = shaderShadow_unis;
  shaderShadow.num_uniforms = 3; 
  ...
  shaderShadow.attributes[NORMAL_INDEX].active = false;
  
  shaderShadow.attributes[TEXEL_INDEX].active = false;

  shaderShadow.attributes[COLOR_INDEX].active = false;
...
void display(void) {     
  ...
  printGLError("display 4:");    
  glCullFace(GL_FRONT);
  printGLError("display 5:");    

 
  glEnable(GL_POLYGON_OFFSET_FILL);
  printGLError("display 6:");    
  glPolygonOffset( 1.1, 4.0);
  printGLError("display 7:");    
  ...
  matrix_translate4x4f(modelViewMatrix,robx,roby,robz);
 
  matrix_rotate4x4f(modelViewMatrix,3.14159f/2.0f, 0.0,1.0,0.0);
  matrix_rotate4x4f(modelViewMatrix,3.14159f/2.0f, 1.0,0.0,0.0);

 
  md2_renderFrameFlat(&shaderShadow,md2_data,curr_frame);
  ...
  */
  ...
  printGLError("display 8 :");    
  glBindFramebuffer(GL_FRAMEBUFFER, 0);
  printGLError("display 9 :");    
  glViewport(0,0,window_x,window_y); 
  printGLError("display 10 :");    
  glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
  printGLError("display 11 :");    
  glEnable(GL_CULL_FACE);
  printGLError("display 12 :");    
  //glDisable(GL_CULL_FACE);
  glCullFace(GL_BACK);
  printGLError("display 13 :");    
  glDisable(GL_POLYGON_OFFSET_FILL);
  printGLError("display 14 :");    
  
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  printGLError("display 15 :");    
  ...
  //matrix_ translate4x4f(modelViewMatrix,0.0f,0.0f,-10.0f);
  //matrix_rotate4x4f(modelViewMatrix,-rotation, 0.0,1.0,0.0);
  ...
  //printGLError("display 15b :");    

  /*
  mesh_render(&shader1,&mausoleum.meshes[1]);
  mesh_render(&shader1,&mausoleum.meshes[2]);   
  mesh_render(&shader1,&mausoleum.meshes[3]);
  mesh_render(&shader1,&mausoleum.meshes[4]);
  mesh_render(&shader1,&mausoleum.meshes[5]);
  mesh_render(&shader1,&mausoleum.meshes[6]);
  mesh_render(&shader1,&mausoleum.meshes[7]); 
  
  mesh_render(&shader1,&mausoleum.meshes[8]); 
  */
  //matrix_scale4x4f(modelViewMatrix,0.15,0.15,0.15);
  ...
  matrix_rotate4x4f(modelViewMatrix,robr,0.0,1.0,0.0);
  matrix_translate4x4f(modelViewMatrix,robx,roby,robz);
  //matrix_translate4x4f(modelViewMatrix,10.0, 0.0,0.0);
  ...
  md2_renderFrameFlat(basic_shader,md2_data,curr_frame);
  ...
  //tools_drawLine(&shader1,&v1,VECTOR_SUB3F(&lightsource1),&c);
...
void init(void)
{
  ...
  //  multimesh_load3ds(&mausoleum, "test4.3ds", MESH_LOAD_NORMALS | MESH_LOAD_TEXELS);
...
void timer(int value) {
  ...
  //ofs += 0.5;
  if ( ++counter == 10) {
    curr_frame = ++curr_frame > 45 ? 40 : curr_frame;
    counter = 0;
  }
...
int main(int argc, char **argv) {
  ...
  md2_data = md2_loadFile("tris.md2");
  md2_printHeaderInfo(md2_data);
  printf("\n");
  md2_printSkinNames(md2_data);
